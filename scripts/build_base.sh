...
dnf5 install -y --setopt=install_weak_deps=0 --skip-unavailable cockpit cockpit-podman cockpit-storaged cockpit-ws cockpit-pcp cockpit-machines cockpit-selinux dnf5-plugins fastfetch firewalld git lm_sensors nfs-utils nss-mdns pcp pcp-selinux podman podman-compose podman-docker samba sysstat tuned wget && dnf clean all
